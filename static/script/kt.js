 ...
 * 人要活得优雅,代码更需要优雅
 * $Id kK.js by @xinghuali<xinghuali@126.com>
 */
...
$(document).ready(function(){
	...
	$("form[mini-form]").on("submit",function(){
	...
	$("[mini-submit],a[mini-submit]").on("click",function(e){
		e.stopPropagation();e.preventDefault();
		window.__MINI_LOAD = window.__MINI_LOAD || false;
		if(window.__MINI_LOAD){ //防止重复提交
			return false;
	...
	$("[mini-iframe]").on("click",function(e){
		e.stopPropagation();e.preventDefault();
		if(!__MINI_CONFIRM(this)){
			return false;
		}
	...
	$("[win-load]").on("click", function(e){
		e.stopPropagation();e.preventDefault();
		var url = $(this).attr("action") ? $(this).attr("action") : $(this).attr("href");
		var w = $(window).width();var h = $(window).height();
		window.open (url, 'YLZ-WIN-Dialog','height='+h+',width='+w+',top=0,left=0,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no') 
	});
	//$("[title]").colorTip();
	//ui:tooltip
	$(document).tooltip({
