...
$(document).ready(
	function() {
		$('#upmodel').flash({
		  swf: weburl+'static/flash/upmx.swf',
		  width: 400,
		  height: 340,
...
function challs_flash_update(){
  ...
  idds=idd.split(",");
	...
	a.parameter="bs=tyi";
	a.typefile=["Images (*.gif,*.jpg,*.png)","*.gif;*.jpg;*.png"];
...
//alert(a);
	arrtmp=a.split("|");	
	document.getElementById('show').innerHTML+='<input value="'+$.trim(arrtmp[0])+'" type=hidden name="data[filesize][]"><input value="'+arrtmp[1]+'" type=hidden name="data[fileurl][]"><input value="'+arrtmp[2]+'" type=hidden name="data[filename][]">';	
}
function challs_flash_onCompleteAll(a){
	loading('文件上传完成，失败'+a+'个，<br>正在保存数据...',300);
	$('#tjsubmit').click();
