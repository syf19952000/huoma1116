...
class Ctl_Member_Company_Diary extends Ctl_Ucenter
{
        ...
        if(!($site_id = (int)$site_id) && !($site_id = (int)$this->GP('site_id'))){
            ...
            if($audit_site < 0){
                ...
                if($status_list[$data['status']]['has_diary']){
                    ...
                    $this->err->set_data('forward', $this->mklink('member/company/diary:site', array($site_id)));
                }
            }
        }else{
            $this->pagedata['site'] = $site;
            ...
            $this->tmpl = 'mobile:member/company/site/diary/create.html';
        }
    }
    ...
    public function edit($item_id=null)
    {
        ...
        if(!($item_id = (int)$item_id) && !($item_id = (int)$this->GP('item_id'))){
            ...
            $this->pagedata['site'] = $site;
            $this->tmpl = 'mobile:member/company/site/diary/edit.html';
        }        
    }
