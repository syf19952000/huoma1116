 ...
 * 人要活得优雅,代码更需要优雅
 * $Id: comment.ctl.php 9372 2015-11-26 06:32:36  xinghuali
 */
...
class Ctl_Member_Company_Comment extends Ctl_Ucenter
{
    ...
    public function company($page=1)
    {
        ...
        $this->tmpl = 'mobile:member/company/comment/items.html';      
    }

	public function listcon($page=1)
    {
        $company = $this->ucenter_company();
        $pager = $filter = array();
        $pager['page'] = $page = max((int)$page, 1);
        ...
        $this->pagedata['pager'] = $pager;
        $this->system->config->get('company_comment');
        $this->tmpl = 'mobile:member/company/comment/listcon.html';      
    }
    ...
    public function detail($comment_id=null)
    {
        ...
        if(!($comment_id = (int)$comment_id) && !($comment_id = (int)$this->GP('comment_id'))){
            ...
            $this->system->config->get('company_comment');
            $this->tmpl = 'mobile:member/company/comment/detail.html';
        }
    }
    ...
    public function reply($comment_id=null)
    {
        ...
        if(!($comment_id = (int)$comment_id) && !($comment_id = (int)$this->GP('comment_id'))){
            ...
            $this->tmpl = 'mobile:member/company/comment/reply.html';
        }
    }
