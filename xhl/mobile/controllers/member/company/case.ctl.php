 ...
 * 人要活得优雅,代码更需要优雅
 * $Id: case.ctl.php 9807 2015-04-22 05:55:25Z maoge $
 */

class Ctl_Member_Company_Case extends Ctl_Ucenter
{
    
    protected $_allower_fields = 'uid,title,mianji,intro,seo_title,seo_keywords,seo_description,logo';
    ...
    public function index($page = 1)
    {
        ...
        $this->tmpl = 'mobile:member/company/case/items.html';
    }
	...
	public function listcon($page = 0)
    {
        ...
        $this->pagedata['pager'] = $pager;
        $this->tmpl = 'mobile:member/company/case/listcon.html';
    }
    ...
    public function create()
    {
        ...
        if($data = $this->checksubmit('data')) {
            if (!$data = $this->check_fields($data, $this->_allower_fields)) {
				...
				if(empty($data['title'])){
					$data['title'] = $company['title'].'案例';
				}
				
                if ($case_id = K::M('case/case')->create($data)) {
					$filelist = explode(',',$filelist);
					if(count($filelist)>0){
						$sql_insert = 'insert into {table}(case_id,title,photo,size,views,orderby,clientip,dateline)values';
						$num =0;
						foreach($filelist as $key=>$fval){
					//		$filetype = substr(strrchr(strtolower($fval),'.'),1); 
							$num++;
							$sql_insert.="(".$case_id.",'".$company['title'].$key."','".$fval."','100','10',50,'".__IP."',".__TIME."),";
						}
						$sql_insert = substr($sql_insert,0,strlen($sql_insert)-1);
						if(K::M('case/photo')->addfile_sql($sql_insert)){
  					          K::M('case/case')->update_last($case_id, 100, $num);
							 K::M('company/company')->update_count($company_id, 'case_num', 1);
							 $this->err->add('添加案例成功');
							 $this->err->set_data('forward', $this->mklink('member/company/case:index', null, array(), true));
						}
					}
                }
            }
        } else {
            $this->tmpl = 'mobile:member/company/case/create.html';
        }
    }
    ...
    public function edit($case_id = null)
    {
        ...
        if (!($case_id = (int) $case_id) && !($case_id = (int)$this->GP('case_id'))) {
            ...
            $this->tmpl = 'mobile:member/company/case/edit.html';
        }
    }
    ...
    public function detail($case_id=null, $page=1)
    {
        ...
        if (!($case_id = (int) $case_id) && !($case_id = (int)$this->GP('case_id'))) {
            ...
            if($items = K::M('case/photo')->items_by_case($case_id, $page, $limit, $count)){
                $this->pagedata['items'] = $items;
                $pager['count'] = $count;
                $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink("member/company/case:detail", array($case_id,'{page}')));
            }            
            $this->pagedata['pager'] = $pager;
            $this->tmpl = 'mobile:member/company/case/detail.html';
        }        
    }

    public function imgcreate($case_id=null, $page=1)
    {
        $company = $this->ucenter_company();
        $pager = array();
        if (!($case_id = (int) $case_id) && !($case_id = (int)$this->GP('case_id'))) {
            ...
            $this->pagedata['pager'] = $pager;
            $this->tmpl = 'mobile:member/company/case/imgcreate.html';
        }        
    }
