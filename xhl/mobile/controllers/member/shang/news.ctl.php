...
class Ctl_Member_Shop_News extends Ctl_Ucenter
{
    ...
    public function index()
    {
        ...
        $this->tmpl = 'mobile:member/shop/news/items.html';
    }
    ...
    public function create()
    {
        ...
        if($data = $this->checksubmit('data')){
            ...
            if($allow_news < 0){
                $this->err->add('您是【'.$shop['group_name'].'】没有权限添加新闻中心', 333);
                ...
                if($news_id = K::M('shop/news')->create($data)){                    
                    $this->err->add('添加内容成功');
                    $this->err->set_data('forward', $this->mklink('member/shop/news:index'));
                }
            }
        }else{
            $this->pagedata['pager'] = $pager;            
            $this->tmpl = 'mobile:member/shop/news/create.html';
        }
    }
    ...
    public function edit($news_id=null)
    {
        ...
        if(!($news_id = (int)$news_id) && !($news_id = (int)$this->GP('news_id'))){
            ...
            if(K::M('member/group')->check_priv($shop['group_id'], 'allow_news') < 0){
                $this->err->add('您是【'.$shop['group_name'].'】没有权限修改新闻中心', 333);
            }else if(!$data = $this->check_fields($data, $this->_allow_fields)){
            ...
            $this->tmpl = 'mobile:member/shop/news/edit.html';
        }
    }
