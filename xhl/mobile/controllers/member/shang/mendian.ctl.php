...
class Ctl_Member_Shop_mendian extends Ctl_Ucenter
{
    ...
    public function index()
    {
        ...
        $this->tmpl = 'mobile:member/shop/mendian/items.html';
    }
    ...
    public function create()
    {
        ...
        if($data = $this->checksubmit('data')){
            ...
            if($allow_mendian < 0){
                $this->err->add('您是【'.$shop['group_name'].'】没有权限添加企业门店', 333);
                ...
                if($mendian_id = K::M('shop/mendian')->create($data)){
                    $this->err->add('添加内容成功');
                    $this->err->set_data('forward', $this->mklink('member/shop/mendian:index'));
                }
            }
        }else{
            $this->pagedata['pager'] = $pager;            
            $this->tmpl = 'mobile:member/shop/mendian/create.html';
        }
    }
    ...
    public function edit($mendian_id=null)
    {
        ...
        if(!($mendian_id = (int)$mendian_id) && !($mendian_id = (int)$this->GP('mendian_id'))){
            ...
            $this->tmpl = 'mobile:member/shop/mendian/edit.html';
        }
    }
