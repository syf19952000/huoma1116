...
class Ctl_Member_Sheji_Zhantai extends Ctl_Ucenter
{
    ...
    public function index()
    {
        ...
        $items = K::M('xiangmu/xiangmu')->items($filter, $orderby, $page, 10, $count);
        $info['info'] = $items;
        $info['error'] = 0;
    ...
    public function create()
    {
        ...
        if($data = $this->checksubmit('data')){
            ...
            $info['error'] = 0;
            $cate = K::M('xiangmu/cate')->tree(null);
            foreach($cate as $k=>$v){
                $info['cate'] .= "<option value=\"".$v['cat_id']."\">".$v['title']."</option>";
                foreach($v['children'] as $kk=>$vv){
                    $info['cate'] .= "<option value=\"".$vv['cat_id']."\">&nbsp;&nbsp;├─".$vv['title']."</option>";
                    foreach($vv['children'] as $kkk=>$vvv){
                        $info['cate'] .= "<option value=\"".$vvv['cat_id']."\">&nbsp;&nbsp;&nbsp;&nbsp;├─".$vvv['title']."</option>";
                    }
                }
            }
            $this->ajaxReturn($info);
    ...
    public function edit($xiangmu_id = null)
    {
        ...
        if (!($xiangmu_id = (int) $xiangmu_id) && !($xiangmu_id = (int)$this->GP('xiangmu_id'))) {
            ...
            if($detail['thumb']){
                ...
                $detail['thumb'] = $attachurl."/".$detail['thumb'];
            }
            $info['detail'] = $detail;
            $info['error'] = 0;
            $cate = K::M('xiangmu/cate')->tree(null);
            foreach($cate as $k=>$v){
                $info['cate'] .= "<option value=\"".$v['cat_id']."\"";
                if($detail['cat_id'] == $v['cat_id']){
                    $info['cate'] .= " selected ";
                }
                $info['cate'] .= ">".$v['title']."</option>\"";
                foreach($v['children'] as $kk=>$vv){
                    $info['cate'] .= "<option value=\"".$vv['cat_id']."\"";
                    if($detail['cat_id'] == $vv['cat_id']){
                        $info['cate'] .= " selected ";
                    }
                    $info['cate'] .= ">&nbsp;&nbsp;├─".$vv['title']."</option>";
                    foreach($vv['children'] as $kkk=>$vvv){
                        $info['cate'] .= "<option value=\"".$vvv['cat_id']."\"";
                        if($detail['cat_id'] == $vvv['cat_id']){
                            $info['cate'] .= " selected ";
                        }
                        $info['cate'] .= ">&nbsp;&nbsp;&nbsp;&nbsp;├─".$vvv['title']."</option>";
                    }
                }
            }
            $this->ajaxReturn($info);
...
//            $this->tmpl = 'member/sheji/zhantai/edit.html';
    ...
    public function delete($xiangmu_id= null)
    {
        $designer = $this->ucenter_designer();
        if (!($xiangmu_id = (int) $xiangmu_id) && !($xiangmu_id = (int)$this->GP('xiangmu_id'))) {
            $this->ajaxReturn(array('error'=>1,'message'=>"项目不存在或已经删除"));
            $this->error(404);
        }else if(!$xiangmu = K::M('xiangmu/xiangmu')->detail($xiangmu_id)){
            $this->ajaxReturn(array('error'=>212,'message'=>"项目不存在或已经删除"));
            $this->err->add('项目不存在或已经删除', 212);
        }else if ($xiangmu['uid'] != $designer['uid']) {
            $this->ajaxReturn(array('error'=>212,'message'=>"不许越权管理别人的内容"));
            $this->err->add('不许越权管理别人的内容', 212);
        }else if(K::M('xiangmu/xiangmu')->delete($xiangmu_id)){
            $this->ajaxReturn(array('error'=>0,'message'=>"删除项目成功"));
            $this->err->add('删除项目成功');
        }
    }
	...
	public function check($xiangmu_id=0,$step=1){
		...
		if ($step==1) {
			...
			//获取传递的host
			$url = 'http://'.$detail['infourl'].'/gdh/authfile/authfile.txt';
