 ...
 * 人要活得优雅,代码更需要优雅
 * $Id: reply.ctl.php 9372 2015-11-26 06:32:36  xinghuali
 */
...
class Ctl_Member_Weixin_Reply extends Ctl_Ucenter 
{
    ...
    public function index($page=1)
    {
        ...
        $this->tmpl = 'member/weixin/reply/items.html';
    }
        ...
        if($data = $this->checksubmit('data')){
            if(!$data = $this->check_fields($data, 'title,intro,content,jumpurl')){
                ...
                if($reply_id = K::M('weixin/reply')->create($data)){
                    $this->err->set_data('forward', $this->mklink('member/weixin/reply:index'));
                    $this->err->add('添加微信素材成功');
                }
            }
        }else{
            $this->tmpl = 'member/weixin/reply/create.html';
        }
    }
    ...
    public function edit($reply_id=null)
    {
        ...
        if(!($reply_id = (int)$reply_id) && !($reply_id = (int)$this->GP('reply_id'))){
            ...
            $this->tmpl = 'member/weixin/reply/edit.html';
        }
    }
    ...
    public function dialog($page=1)
    {
        ...
        $this->pagedata['pager'] = $pager;
        $this->tmpl = 'member/weixin/reply/dialog.html';
    }
