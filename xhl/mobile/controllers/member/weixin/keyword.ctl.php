 ...
 * 人要活得优雅,代码更需要优雅
 * $Id: keyword.ctl.php 9372 2015-11-26 06:32:36  xinghuali
 */

class Ctl_Member_Weixin_Keyword extends Ctl_Ucenter
{
    ...
    public function index($page=1)
    {
        ...
        $this->tmpl = 'member/weixin/keyword/items.html';    
    }
        ...
        if($data = $this->checksubmit('data')){
            if(!$data = $this->check_fields($data, 'keyword,reply_id,content')){
                ...
                if($kw_id = K::M('weixin/keyword')->create($data)){
                    $this->err->set_data('forward', $this->mklink('member/weixin/keyword:index'));
                    $this->err->add('添加关键字成功');
                }
            }
        }else{
            $this->tmpl = 'member/weixin/keyword/create.html';
        }
    }
    ...
    public function edit($kw_id=null)
    {
        ...
        if(!($kw_id = (int)$kw_id) && !($kw_id = (int)$this->GP('kw_id'))){
            ...
            $this->tmpl = 'member/weixin/keyword/edit.html';
        }
    }
