 ...
 * 人要活得优雅,代码更需要优雅
 * $Id: menu.ctl.php 9372 2015-11-26 06:32:36  xinghuali
 */
...
class Ctl_Member_Weixin_Menu extends Ctl_Ucenter
{
    ...
    public function index()
    {
        ...
        $this->tmpl = 'member/weixin/menu/items.html';       
    }
        ...
        if($data = $this->checksubmit('data')){
            if(!$data = $this->check_fields($data, 'title,parent_id,type,reply_id,content,link,orderby')){
                ...
                if($menu_id = K::M('weixin/menu')->create($data)){
                    $this->err->set_data('forward', $this->mklink('member/weixin/menu:index'));
                    $this->err->add('添加微信菜单成功');
                }
            }
        }else{
            $this->pagedata['wx_menu_list'] = K::M('weixin/menu')->items_by_weixin($weixin['wx_id']);
            $this->tmpl = 'member/weixin/menu/create.html';
        }
    }
    ...
    public function edit($menu_id=null)
    {
        ...
        if(!($menu_id = (int)$menu_id) && !($menu_id = (int)$this->GP('menu_id'))){    
            ...
            $this->pagedata['wx_menu_list'] = K::M('weixin/menu')->items_by_weixin($weixin['wx_id']);
            $this->tmpl = 'member/weixin/menu/edit.html';
        }
    }
