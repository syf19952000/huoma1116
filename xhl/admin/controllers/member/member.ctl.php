 ...
 * $Id: member.ctl.php 2016-09-27 02:07:36  xinghuali
 */
...
class Ctl_Member_Member extends Ctl
{
    public function index($page=1)
    {
       ...
       // }
        $items = K::M('member/member')->user_all();
...
>>>>>>> d2a31ff35db971065813ae1c59dd238b6ff6aa58
    ...
    public function dialog($from='designer', $page=1)
    {
        $filter = $pager = array();
        $pager['page'] = max(intval($page), 1);
        $pager['from'] = 'all';
    ...
    public function manager($uid)
    {
        if(K::M('member/auth')->manager($uid)) {
            ...
            $link = $cfg['siteurl'].'/index.php?member/member-index.html';
    ...
    public function gold($uid=null)
    {
        if(!($uid = (int)$uid) && !($uid = (int)$this->GP('uid'))){
            ...
            if(!$data = $this->GP('data')){
                ...
                $this->err->add('减去的展币不能大于现有展币', 216);
