 ...
 * 人要活得优雅,代码更需要优雅
 * $Id: look.ctl.php 10053 2015-05-05 14:18:09Z maoge $
 */
        ...
        if(empty($tenders_id)){
            ...
            if($this->checksubmit()){
                if(!$data = $this->GP('data')){
					...
					$this->err->add('该工厂已投过标', 201)->response();
				}else{
					...
					if($tenders_look == true && $look_id = K::M('tenders/look')->create($datas)){
						...
						$smsdata = $maildata = array('contact'=>$detail['contact'] ? $detail['contact'] : '参展商','mobile'=>$detail['mobile']);
