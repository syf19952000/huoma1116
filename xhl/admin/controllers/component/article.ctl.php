 ...
 * 人要活得优雅,代码更需要优雅
 * $Id: article.ctl.php 2016-09-27 02:07:36  xinghuali
 */
...
class Ctl_Component_article extends Ctl
{
    public function items($page=1)
    {
		...
		$this->tmpl = 'admin:component/article/items.html';
		
    }
    public function index($uid,$page=1)
    {
		if(!$detail = K::M('component/component')->detail($uid)){
            $this->err->add('该项目不存在或已经删除', 212);
    ...
    public function create($uid)
    {
		if(!$uid && !$uid = $this->GP('uid')){
			...
			if($data = $this->checksubmit('data')){
				...
				$this->pagedata['uid'] = $uid;
			    $this->tmpl = 'admin:component/article/create.html';
			}
		}
    }
    ...
    public function edit($article_id=null)
    {
        if(!($article_id = (int)$article_id) && !($article_id = $this->GP('article_id'))){
        	...
        	$this->tmpl = 'admin:component/article/edit.html';
        }
    }
    ...
    public function so()
    {
        $this->tmpl = 'admin:component/article/so.html';
    }
