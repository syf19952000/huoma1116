...
class Ctl_Xiangmu_article extends Ctl
{
    public function items($page=1)
    {
		...
		$this->tmpl = 'admin:xiangmu/article/items.html';
		
    }
    public function index($uid,$page=1)
    {
		if(!$detail = K::M('xiangmu/xiangmu')->detail($uid)){
    ...
    public function create($uid)
    {
		if(!$uid && !$uid = $this->GP('uid')){
			...
			if($data = $this->checksubmit('data')){
				...
				$this->pagedata['uid'] = $uid;
			    $this->tmpl = 'admin:xiangmu/article/create.html';
			}
		}
    }
    ...
    public function edit($article_id=null)
    {
        if(!($article_id = (int)$article_id) && !($article_id = $this->GP('article_id'))){
        	...
        	$this->tmpl = 'admin:xiangmu/article/edit.html';
        }
    }
    ...
    public function so()
    {
        $this->tmpl = 'admin:xiangmu/article/so.html';
    }
