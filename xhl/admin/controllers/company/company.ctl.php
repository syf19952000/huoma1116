 ...
 * 人要活得优雅,代码更需要优雅
 * $Id: company.ctl.php 2016-09-27 02:07:36  xinghuali
 */
...
class Ctl_Company_Company extends Ctl
{
    ...
    public function dialog($page=1)
    {
        ...
        if($items = K::M('company/company')->items($filter, array('qy_num'=>'DESC','bj_num'=>'DESC','group_id'=>'ASC','company_id'=>'ASC'), $page, $limit, $count)){
            $pager['count'] = $count;
    ...
    public function create()
    {
        if($this->checksubmit()){
            if(!$data = $this->GP('data')){
				...
				$data['zhixing_id'] = $this->admin->admin_id;
				$data['zhixing_name'] = $this->admin->admin_name;
            ...
            $this->tmpl = 'admin:company/company/createnew.html';
        }
    }
...
/*    public function create()
    ...
    public function qian($company_id = null){
         if ($company_id = (int) $company_id) {
             if($detail = K::M('company/company')->detail($company_id)){
                $data['is_qian'] = empty($detail['is_qian']) ? 1 : 0;
                if (K::M('company/company')->update($company_id,$data)) {
                    $this->err->add('设置成功');
                }else{
                     $this->err->add('设置失败');
