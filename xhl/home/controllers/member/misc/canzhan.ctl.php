...
class Ctl_Member_Misc_Canzhan extends Ctl_Ucenter
{
	...
	public function index($page=1)
	{
        ...
        $this->tmpl = 'member/misc/canzhan/items.html';
	}
    ...
    public function detail($id=null)
    {
        if(!$id = (int)$id){
            ...
            K::M('canzhan/canzhan')->update_count($id, 'views');
            $this->tmpl = 'member/misc/canzhan/detail.html';
        }
    }

    public function looked($type=1,$page=1)
    {
        $filter = $pager = array();
        $pager['page'] = max(intval($page), 1);
        ...
        $this->pagedata['items'] = $items;
        $this->pagedata['pager'] = $pager;
        $this->tmpl = 'member/misc/canzhan/looks.html';
    }
    ...
    public function track($look_id=null, $page=1)
    {
        if(!$look_id = (int)$look_id){
            ...
            $canzhan_case = K::M('canzhan/case')->items(array('look_id'=>$look_id), array('dateline'=>'DESC'));
			if(count($canzhan_case>0)){
				$czc_list =array();
				foreach($canzhan_case as $val){
					$val['cphoto'] = K::M('canzhan/photo')->items(array('case_id'=>$val['case_id'],'closed'=>0),array('dateline'=>'DESC'));
					$czc_list[] = $val;
				}
			}
            ...
            $this->pagedata['pager'] = $pager;
            $this->tmpl = 'member/misc/canzhan/track.html';
        }  
    }
    ...
    public function look($id=null)
    {
        if(!($id = (int)$id) && !($id = (int)$this->GP('id'))){
            ...
            $this->tmpl = 'member/misc/canzhan/look.html';
        }
    }
