...
class Init_Log{
    ...
    protected function __construct(){//注意：以下是配置文件中的常量，请读者自行更改
        ...
        $this->log_file_path     = dirname(ROOT_PATH)."/files/log/";
    ...
    public function log($type,$desc,$time){
        if($this->log_switch){
            ...
            switch($type){
                case 0:
                    fwrite(self::$handle, 'THING LOG' . ' ' . $time  . ': ' . $desc. chr(13) );
                    break;
                case 1:
                    fwrite(self::$handle, 'ERROR LOG' . ' ' . $time  . ': ' . $desc . chr(13));
                    break;
                default:
                    fwrite(self::$handle, 'INFO LOG' . ' ' . $time  . ': ' . $desc . chr(13));
                    break;
            }
