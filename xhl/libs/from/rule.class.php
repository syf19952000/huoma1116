...
Class rule{
    ...
    protected $rulearray=array('min_length','max_length');
    ...
    public function dorule()
    {
        ...
        foreach ($rule as $k => $v){
            foreach ($v as $kk => $vv){
                if(in_array($vv,$this->rulearray)){
                    if(!$this->$vv($data[$k],$kk)){
                        return false;
                    }
                }else{
                    if(!$this->$vv($data[$k])){
    ...
    function required($val)
    {
        if(is_array($val) && !count($val)) {
            return false;
        }elseif((is_string($val) && trim($val) === '') || empty($val)){
            return false;
        }else{
            return true;
        }
    ...
    */
    ...
    function valid_mobile($mobile){
        ...
        if(preg_match('/13[0-9]\d{8}|14[0-9]\d{8}|15[0|1|2|3|5|6|7|8|9]\d{8}|17[0-9]\d{8}|18[0|5|6|7|8|9]\d{8}/',$mobile)){
            return true;
        }else{
            return false;
        }
