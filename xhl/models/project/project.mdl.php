 ...
 * 人要活得优雅,代码更需要优雅
 * $Id: project.mdl.php 2015-09-27 02:07:36  xinghuali
 */
...
class Mdl_Project_Project extends Mdl_Table
{   
  
    protected $_table = 'xiangmu_project';
    protected $_pk = 'project_id';
    protected $_cols = 'project_id,xiangmu_id,group_id,company_id,city_id,area_id,school,qq,qqpas,case_num,blog_num,attention_num,yuyue_num,tenders_num,tenders_sign,views,comments,score,score1,score2,score3,slogan,about,orderby,flushtime,audit,closed,clientip,dateline,mobile,is_qian,skills';
    ...
    {
        $where = $this->where($filter, 'x.');
        $orderby = $this->order($orderby, null, 'd.');
        $limit = $this->limit($p, $l);
        $items = array();
        $sql = "SELECT SQL_CALC_FOUND_ROWS d.*,x.favorites,x.comments FROM ".$this->table($this->_table)." d LEFT JOIN ".$this->table('xiangmu')." AS x ON x.xiangmu_id=d.xiangmu_id WHERE $where $orderby $limit";
    ...
    public function detail($project_id, $closed=false)
    {
        ...
        $sql = "SELECT x.*,d.* FROM ".$this->table($this->_table)." d LEFT JOIN ".$this->table('xiangmu')." x ON x.xiangmu_id=d.xiangmu_id WHERE $where";
    ...
    public function info($project_id, $closed=false)
    {
        if(!$project_id = (int)$project_id){
            return false;
        }
        ...
        $sql = "SELECT d.* FROM ".$this->table($this->_table)." d WHERE $where";
        if($row = $this->db->GetRow($sql)){
            $row = $this->_format_row($row);
        }
        return $row;
    ...
    public function shejidetail($uid)
    { 
     ...
     public function items_by_getone($project_id)
    {
        $where = " d.project_id = ".$project_id;
        $items = array();
        $sql = "SELECT SQL_CALC_FOUND_ROWS d.*,x.uid,x.comments FROM ".$this->table($this->_table)." d LEFT JOIN ".$this->table('xiangmu')." AS x ON x.xiangmu_id=d.xiangmu_id WHERE $where ";
    ...
    public function project_find($project_id)
    {
    ...
    public function project_all($xiangmu_id)
    {

        $where = " where xiangmu_id = ".$xiangmu_id." order by project_id desc";
