...
{     
	...
	public function upload($attach, $from='editor', $source=null)
	{
        ...
        if($file = K::M('helper/upload')->upload($attach, $dir, $fname)){
            ...
            $hash = md5($photo);
            $a = array('size'=>$attach['size'], 'photo'=>$photo,'name'=>$attach['name'], 'hash'=>$hash, 'type'=>$attach['type']);
            if($item){
	...
	public function upload_mobile($attach, $from='editor', $source=null)
	{
        $ym = date('Ym', __CFG::TIME);
        $cfg = K::$system->config->get('attach');
        $dir = $cfg['attachdir'].'photo'.DIRECTORY_SEPARATOR.$ym.DIRECTORY_SEPARATOR;
        $dir_tmp = $cfg['attachdir'].'photo'.DIRECTORY_SEPARATOR.'mobile_tmp'.DIRECTORY_SEPARATOR;
        if($source && is_string($source) && strlen($source) > 32){
            if(!preg_match('/^(http|temp|cache)/', $source)){
                $hash = md5($source);
                if($item = $this->item_by_hash($hash)){
        ...
        if($file = K::M('helper/upload')->upload_mobile($attach, $dir,$dir_tmp, $fname)){
        	$photo = "photo/{$ym}/{$fname}";
            $hash = md5($photo);
            $a = array('size'=>$attach['size'], 'photo'=>$photo,'name'=>$attach['name'], 'hash'=>$hash, 'type'=>$attach['type']);
            if($item){
	...
	public function upload_jihua($attach, $from='editor', $source=null)
	{
        $ym = date('Ym', __CFG::TIME);
        $cfg = K::$system->config->get('attach');
        $dir = $cfg['attachdir'].'jihua'.DIRECTORY_SEPARATOR.$ym.DIRECTORY_SEPARATOR;
        if($file = K::M('helper/upload')->upload($attach, $dir, $fname)){
        	$photo = "jihua/{$ym}/{$fname}";
            $hash = md5($photo);
            $a = array('size'=>$attach['size'], 'photo'=>$photo,'name'=>$attach['name'], 'hash'=>$hash, 'type'=>$attach['type']);
        	return $a;
        }
