 ...
 * $Id: cate.mdl.php 2015-09-27 02:07:36  xinghuali
 */
...
class Mdl_Xiangmu_Cate extends Mdl_Table
{   
  
    protected $_table = 'xiangmu_cate';
    protected $_pk = 'cat_id';
    protected $_cols = 'cat_id,parent_id,title,seo_title,seo_keywords,seo_description,level,from,bgimg,orderby,hidden,dateline';
    protected $_orderby = array('parent_id'=>'ASC','orderby'=>'ASC','cat_id'=>'ASC');
    protected $_pre_cache_key = 'xiangmu-cate-list';

    protected static $_allow_from = array('about','help','page','xiangmu');
    ...
    public function fetch_all()
    {
        if($this->_pre_cache_key === null){
           ...
           // $sql = "SELECT * FROM ".$this->table($this->_table)." where parent_id=2 ORDER BY level ASC,orderby ASC,cat_id ASC";
    ...
    public function fenlei_all()
    {
        $sql = "SELECT * FROM ".$this->table($this->_table)." where parent_id = 0 ORDER BY level ASC,orderby ASC,cat_id ASC";
        if($rs = $this->db->Execute($sql)){
             while($row = $rs->fetch()){
                $items[$row[$this->_pk]] = $row;
             }
        ...
        foreach ($items as $kk => $vv) {
             $sql = "SELECT * FROM ".$this->table($this->_table)." where parent_id = ".$vv['cat_id']." ORDER BY level ASC,orderby ASC,cat_id ASC";
             if($rs = $this->db->Execute($sql)){
             while($row = $rs->fetch()){
                $items[$kk]['son'][$row[$this->_pk]] = $row;
             }
        }
        }
        return $items;        
    ...
    protected function _check($data, $cid=null)
    {
        ...
        if(isset($data['from'])){
            $data['from'] = in_array($data['from'], self::$_allow_from) ? $data['from'] : 'xiangmu';
        }
    ...
    public function hangye_all()
    {
          $sql = "SELECT * FROM ".$this->table($this->_table)."  where parent_id = 21 ORDER BY level ASC,orderby ASC,cat_id ASC";
           if($rs = $this->db->Execute($sql)){
                while($row = $rs->fetch()){
                    $items[$row[$this->_pk]] = $row;
                }
    ...
    public function yuyan_all()
    {
          $sql = "SELECT * FROM ".$this->table($this->_table)."  where parent_id = 12 ORDER BY level ASC,orderby ASC,cat_id ASC";
           if($rs = $this->db->Execute($sql)){
                while($row = $rs->fetch()){
                     $items[$row[$this->_pk]] = $row;
    ...
    public function yuanxing_all()
    {
          $sql = "SELECT * FROM ".$this->table($this->_table)."  where parent_id = 13 ORDER BY level ASC,orderby ASC,cat_id ASC";
           if($rs = $this->db->Execute($sql)){
                while($row = $rs->fetch()){
                  $items[$row[$this->_pk]] = $row;
                }
    ...
    public function list_search($cat_id)
    {

        $sql = "SELECT * FROM ".$this->table($this->_table)."  where parent_id = ".$cat_id;
           if($rs = $this->db->Execute($sql)){
                while($row = $rs->fetch()){
                  $items[$row[$this->_pk]] = $row;
                }
