...
class Mdl_Component_Component extends Mdl_Table
{   

    protected $_table = 'component';
    protected $_pk = 'component_id';
    protected $_cols = 'component_id,cat_id,from,page,title,thumb,yuyan,hangye,chajian,huanjing,ontime,size,xingzhi,gurl,xnum,url,linkurl,desc,views,favorites,comments,orderby,dateline,audit,hidden,closed,uid,infourl,xmprice';
    protected $_orderby = array('orderby'=>'ASC', 'component_id'=>'DESC');

    protected $_hot_orderby = array('views'=>'DESC', 'orderby'=>'ASC');
    protected $_hot_filter = array('from'=>'component','hidden'=>'0', 'audit'=>'1', 'closed'=>'0');
    protected $_new_orderby = array('component_id'=>'DESC');
    protected $_new_filter = array('from'=>'component','hidden'=>'0', 'audit'=>'1', 'closed'=>'0');
    ...
    public function create($data)
    {
        ...
        if($xiangmu_id = $this->db->insert($this->_table, $data, true)){
            K::M('component/content')->create($xiangmu_id, $this->xiangmu_ext);
            //正则获取thumb,photoIds
            if(preg_match_all("/(photo\/\d+\/\d{8}_[\dA-F]{32}\.(jpg|gif|png|jpeg))\?PID(\d+)/i", $this->xiangmu_ext['content'], $matches)){
                ...
                if($count = K::M('component/photo')->update_by_xiangmu($xiangmu_id, $pids)){
                    $a['photos'] = $count;
                }
                if($a){
    ...
    public function update($xiangmu_id, $data, $checked=false)
    {
        ...
        if($this->xiangmu_ext){
            K::M('component/content')->update($xiangmu_id, $this->xiangmu_ext);
        }
        return $ret;
    }
    ...
    public function xiangmudetail($xiangmu_id, $closed=false)
    {
        ...
        $sql = "SELECT c.*,a.* FROM ".$this->table($this->_table)." a LEFT JOIN ".$this->table('component_comment')." c ON a.xiangmu_id=c.xiangmu_id WHERE $where LIMIT 1";
        if($xiangmudetail = $this->db->GetRow($sql)){
            $cate = K::M('component/cate')->cate($xiangmudetail['cat_id']);
    ...
    public function prev_item($xiangmu_id, $cat_id=0)
    {
        ...
        $sql = "SELECT * FROM ".$this->table($this->_table)." WHERE $where xiangmu_id<$xiangmu_id AND `from`='component' AND closed=0 ORDER BY xiangmu_id DESC LIMIT 1";
    ...
    public function next_item($xiangmu_id, $cat_id=0)
    {
        ...
        $sql = "SELECT * FROM ".$this->table($this->_table)." WHERE $where xiangmu_id>$xiangmu_id AND `from`='component' AND closed=0 ORDER BY xiangmu_id ASC LIMIT 1";
        return $this->db->GetRow($sql);
    }
    ...
    public function item_by_page($page,$city_id=0)
    {
        ...
        $sql = "SELECT c.*,a.* FROM ".$this->table($this->_table)." a  LEFT JOIN ".$this->table('component_content')." c ON a.xiangmu_id=c.xiangmu_id  WHERE $where";
    ...
    public function about($page, $city_id=0)
    {
        ...
        $sql = "SELECT c.*,a.* FROM ".$this->table($this->_table)." a LEFT JOIN ".$this->table('component_content')." c ON a.xiangmu_id=c.xiangmu_id WHERE a.page='{$page}' AND a.from='about' LIMIT 1";
        return $this->db->GetRow($sql);
    }
    ...
    public function help($page, $city_id=0)
    {
        ...
        $sql = "SELECT c.*,a.* FROM ".$this->table($this->_table)." a LEFT JOIN ".$this->table('component_content')." c ON a.xiangmu_id=c.xiangmu_id WHERE a.page='{$page}' AND a.from='help' LIMIT 1";
        return $this->db->GetRow($sql);        
    }
    ...
    {
        if($cate = K::M('component/cate')->cate($row['cat_id'])){
    ...
    protected function _check($data, $xiangmu_id=null)
    {
        ...
        if(isset($data['from'])){
            if(!in_array($data['from'], array('component', 'about', 'help', 'page'))){
                $data['from'] = 'component';
            }
        }
