...
class Mdl_Component_Cate extends Mdl_Table
{   
  
    protected $_table = 'component_cate';
    ...
    protected $_pre_cache_key = 'component-cate-list';

    protected static $_allow_from = array('about','help','page','component');
    ...
    public function fenlei_all()
    {
        ...
        if($rs = $this->db->Execute($sql)){
             while($row = $rs->fetch()){
                $items[$row['parent_id']][$row[$this->_pk]] = $row;
             }
        }
        return $items;        
    }
    ...
    protected function _check($data, $cid=null)
    {
        ...
        if(isset($data['from'])){
            $data['from'] = in_array($data['from'], self::$_allow_from) ? $data['from'] : 'component';
        }
