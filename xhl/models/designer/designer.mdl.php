 ...
 * 人要活得优雅,代码更需要优雅
 * $Id: designer.mdl.php 2015-09-27 02:07:36  xinghuali
 */
...
{   
    ...
    protected $_pk = 'uid';
    protected $_cols = 'uid,name,group_id,company_id,province_id,city_id,area_id,school,qq,qqpas,case_num,blog_num,attention_num,yuyue_num,tenders_num,tenders_sign,views,comments,score,score1,score2,score3,slogan,about,orderby,flushtime,audit,closed,clientip,dateline,mobile,is_qian,skills';
    ...
    public function xiangmu_count($uid = 0){
        $sql = "SELECT COUNT(*) FROM ".$this->table("xiangmu")." WHERE uid='".$uid."'";
        return $this->db->GetOne($sql);
    }

    public function follow_count($uid = 0){
        $sql = "SELECT COUNT(*) FROM ".$this->table("xiangmu_sheji")." WHERE uid='".$uid."'";
        return $this->db->GetOne($sql);
    }

    public function fans_count($uid = 0){
        $sql = "SELECT COUNT(*) FROM ".$this->table("xiangmu_sheji")." WHERE xiangmu_id='".$uid."'";
        return $this->db->GetOne($sql);
    }
