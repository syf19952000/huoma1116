 ...
 * $Id: link.mdl.php 9877 2015-04-24 04:08:43  xinghuali
 */
...
class Mdl_Helper_Link extends Model
{
    ...
    public function mklink($ctl, $args=array(), $params=array(), $http=false, $rewrite=true, $ext='.html')
    {
        ...
        if(defined('IN_YUANGONG')){
            $link = "yiluzhan/index.php?{$link}";
        }else if('ajax' === $http || defined('IN_ADMIN')){
            $link = "ezhan/index.php?{$link}";
        }
        if($params){
...
//            $prefix = $_CFG['site']['siteurl'];
            ...
            }else if($_CFG['site']['multi_city'] /*  && in_array($ctl, $this->_ctl)*/){
                $city = $request['city'];
                $prefix = $city['siteurl'];
            }
            $link = $prefix.'/'.$link;
    ...
    {
        if(substr($ctl, 0, 8) == 'member/' || substr($ctl, 0, 7) == 'weixin/' || substr($ctl, 0, 7) == 'mobile/'){
            $route_type = 0;
        }else{
