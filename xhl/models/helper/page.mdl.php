 ...
 * $Id page.mdl.php xinghuali<xinghuali@126.com>
 */
...
class Mdl_Helper_Page
{
	...
	public function mkpage($count, $limit, $curpage=1, $link=null, $params=array())
	{
		...
		if($count > $limit) {
			...
			if(!defined('IN_MOBILE')){
			$multipage = ($curpage > 1 ? '<li><a href="'.$this->format_link($link, ($curpage - 1)).'" title="上一页" class="prev">上一页</a></li>' : '').($curpage - $offset > 1 && $pages > $page ? '<a href="'.$this->format_link($link, 1).'" class="first">1 ...</a>' : '');
			}
			for($i = $from; $i <= $to; $i++) {
				$multipage .= $i == $curpage ? '<li><a href="javascript:vido(0);"><strong>'.$i.'</strong></a></li>' :
					'<li><a href="'.$this->format_link($link, $i).'">'.$i.'</a></li>';
			}
			if(!defined('IN_MOBILE')){
			$multipage .= ($to < $pages ? '<li><a href="'.$this->format_link($link, $pages).'" class="last">... '.$pages.'</a></li>' : '').($curpage < $pages ? '<li><a href="'.$this->format_link($link, ($curpage + 1)).'" title="下一页" class="next">下一页</a></li>' : '');
			}
			if(defined('IN_ADMIN') || defined('IN_YUANGONG')){
