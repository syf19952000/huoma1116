 ...
 * $Id: block.mdl.php 10073 2015-05-06 07:07:50Z wanglei $
 */
...
{   
    ...
    protected static $_allow_from = array('chao'=>'特装', 'designer'=>'设计师','company'=>'工厂', 'shang'=>'参展商','news'=>'工厂新闻','case'=>'案例','diary'=>'日记','article'=>'文章','guan'=>'展馆','zhan'=>'展会');
    ...
    public function load_mdl($from)
    {
        static $_allow_mdl = array(
            ...
            'guan'=>'guan/guan','company'=>'company/company',
            'shang'=>'shang/shang', 'zhan'=>'zhan/zhan', 'coupon'=>'shop/coupon',
    ...
    public function format_item($row, $from='product')
    {
        ...
        if(in_array($from, array('mechanic', 'designer'))){
            ...
            $row['itemId'] = $row['guan_id'];
        }else if('zhan' == $from){
            $row['itemId'] = $row['zhan_id'];
        }else if('tuan' == $from){
    ...
    public function block_items($block, $limit=null)
    {
        $block_id = $block['block_id'];
        $limit = (int)$limit ? (int)$limit : $block['limit'];
        $cache_key =$this->_pre_cache_key."-items-{$block_id}-{$limit}";
        if(!$block_items = $this->cache->get($cache_key, $block['ttl'])){
            $block_items = array();
            if(!$mdl = $this->load_mdl($block['from'])){
                return false;
            }
            if($items = K::M('block/item')->items(array('block_id'=>$block_id), null, 1, $limit)){
                $iids = array();
                $time = __CFG::TIME - 86400;
                $has_expire = false;
                $count = 0;
...
//        $city_id = (int)$params['city_id'];
		...
		if($items = $this->block_items($block, $limit)){
            $content = '';
            $index = 0;
            $iteration = 1;
            $count = count($items);
    ...
    protected function _format_link($row, $block)
    {
        ...
        switch ($block['from']) {
            case 'designer':
                $link = $oLink->mklink('sheji/shicenter', array($row['itemId']), array(), $http);
                break;
            case 'mechanic':
            ...
            case 'company': 
                if(!$link = $row['company_url']){
                    $link = $oLink->mklink('gong/chang', array($row['itemId']), array(), $http);
                }
                break;
            case 'news':
            ...
            case 'shop':
                ...
                break;
            case 'coupon':
                $link = $oLink->mklink('qiye/coupon:detail', array($row['itemId']), array(), $http);
                break;
            case 'product':
                $link = $oLink->mklink('qiye/product:detail', array($row['itemId']), array(), $http);
                break;          
            case 'activity':
