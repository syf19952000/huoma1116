 ...
 * $Id mysqli.mdl.php xinghuali<xinghuali@126.com>
 */

class Mdl_Mysql_Mysqli
{
    ...
    public function connect()
    {
        if($this->link === null){
            ...
            if(!$this->link = mysqli_connect($this->cfg['host'], $this->cfg['user'], $this->cfg['passwd'], $this->cfg['dbname'], $this->cfg['port'], $this->cfg['socket'])){
                if(__CFG::DEBUG){
                    trigger_error('Mysql Connect:['.mysqli_errno().']'.mysqli_error());
                }
...
/*        $this->_QSQL[] = $sql;
     ...
     * 同Adodb:GetRow,兼容用
     */
    public function GetRow_arr($sql)
    {
        if(!preg_match("/LIMIT/i",$sql)){
            $sql = "$sql LIMIT 1";
        }
...
//		echo "$cmd $table SET $sql WHERE $where";die;
 		$table = $this->table($table);
...
/*       if (self::$config['fullnotice'] && strpos($clean, 'like0x')) {
            return '-2';
        }*/
