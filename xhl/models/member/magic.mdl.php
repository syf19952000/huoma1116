 ...
 * 人要活得优雅,代码更需要优雅
 * $Id: magic.mdl.php 2016-09-27 02:07:36  xinghuali
 */
...
class Mdl_Member_Magic extends Mdl_Member_Member
{
    ...
    public function verify_name($uids, $verify=true)
    {
        ...
        foreach($member_list as $v){
            ...
            }else if($v['from'] == 'hotel'){
                $hotel_uids[$v['uid']] = $v['uid'];
            }
        }
        ...
        if($this->db->Execute($sql)){
            ...
            if($hotel_uids){
                K::M('hotel/hotel')->verify_name($hotel_uids, $verify);
            }
            if($shop_uids){
    ...
    public function send_verify_mail($uid)
    {
        if($member = K::M('member/member')->member($uid)){
            ...
            $link = K::M('helper/link')->mklink('user:verfiy', "mail-{$member['uid']}-{$token}", array(), 'www');
    ...
    public function send_forgot($mail)
    {
        if(!K::M('verify/check')->mail($mail)){
            ...
            $link = K::M('helper/link')->mklink('user:forgot', "reset-{$member['uid']}-{$token}", array(), 'www');
            return K::M('helper/mail')->sendmail($member['mail'], 'member_forgot', array('uname'=>$member['uname'], 'forgot_link'=>$link));
        }
        return false;
    }
    public function forgot_mobile($mobile)
    {
