 ...
 * $Id: auth.mdl.php 2016-09-27 02:07:36  xinghuali
 */
...
{   
     ...
     * @param   $p  密码{明文密码}
     */
    public function login($u, $p, $l=null, $ismd5=false, $keep=false,$app='',$openid='')
    {
        $passwd =$ismd5 ? $p : md5($p);
//        if($l === null){
            if(K::M('verify/check')->mobile($u)){
                $l = 'mobile';
            }elseif(K::M('verify/check')->mail($u)){
                $l = 'mail';
            }else{
                $l = 'uname';
            }
        ...
        if(!$m = K::M('member/member')->member($u, 'mobile')){
			...
			if(defined('UC_OPEN') && UC_OPEN){
				...
				if($uc['uid'] > 0){
					...
					if($app){
						K::M('member/member')->update($m['uid'], array('lastlogin'=>__CFG::TIME, 'loginip'=>__IP,'app'=>$app,'openid'=>$openid,'islogin'=>1), true);
					}else{
            ...
            $this->cookie->set('uid', $m['uid'], $expire);
            return $m;
        }
        return false;       
    }
