 ...
 * 人要活得优雅,代码更需要优雅
 * $Id: qqlogin.mdl.php 10098 2015-05-06 14:44:00  xinghuali
 */
class Mdl_Member_Qqlogin extends Model
{
    ...
    public function qqloign_url()
    {
        ...
        if(defined('IN_MOBILE')){
            ...
            $site = K::$system->config->get('site');
            $callback = $site['siteurl'] . '/' . K::M('helper/link')->mklink('user:qqcallback');
        }
    ...
    public function qqcallback($code, $state)
    {
        ...
        if(defined('IN_MOBILE')){
            ...
            $site = K::$system->config->get('site');
            $callback = K::M('helper/link')->mklink('user:qqcallback', array(), array(), 'www');
        }
        if (empty($cfg['qq_is_open'])) {
            $this->err->add('很抱歉网站管理员还未开启QQ登录功能', 201);
            return false;
    ...
    {
        if(!$content = K::M('connect/connect')->detail_by_openid($this->_type_id,$user['openid'])){
            $content['id'] = K::M('connect/connect')->create(array('type'=> $this->_type_id,'open_id'=> $user['openid']));
        }
        if($content['uid'] && $m = K::M('member/member')->member($content['uid'])){
            K::M('member/auth')->manager($m['uid']);
            return true;
        }else if(K::$system->uid){
            if($content['connect_id']){
                K::M('connect/connect')->update($content['connect_id'], array('uid'=>K::$system->uid, 'dateline'=>__TIME), true);
            }else{
...
//                 'passwd'      => substr(md5($uinqid),rand(5, 20),7),
           $a = array(
            ...
            if($uid = K::M('member/account')->create($a)){
                ...
                if($face = file_get_contents($info['figureurl_2'])){
