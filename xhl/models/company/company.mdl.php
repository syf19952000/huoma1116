 ...
 * 人要活得优雅,代码更需要优雅
 * $Id: company.mdl.php 2015-09-27 02:07:36  xinghuali
 */
...
{   
    ...
    protected $_cols = 'company_id,uid,group_id,province_id,city_id,area_id,domain,title,name,thumb,logo,slogan,contact,phone,mobile,qq,addr,name,addr,xiaobao,lng,lat,score,score1,score2,score3,score4,score5,comments,tenders_num,tenders_sign,case_num,news_num,youhui_num,yuyue_num,last_case,site_num,last_site,views,verify_name,is_vip,is_qian,orderby,audit,video,closed,flushtime,fp_num,bj_num,qy_num,clientip,dateline';
    ...
    public function items_by_compay_ids($ids)
    {
        if(!$uids = K::M('verify/check')->ids($ids)){
            return false;
        }
        $order = $this->order();
        $sql = "SELECT company_id,title,logo,mobile,contact FROM ".$this->table($this->_table)." WHERE company_id IN($uids) AND closed=0 $order";
        $items = array();
        if($rs = $this->db->Execute($sql)){
            while($row = $rs->fetch()){
                $items[$row['company_id']] = $this->_format_row($row);
            }
        }
        return $items;
    }
        ...
        if($uid = (int)$data['uid']){
            if($member = K::M('member/member')->detail($uid)){
                if($member['from'] != 'company'){
                    $this->err->add("所属会员必须为装修工厂类型", 451);
                    return false;
    ...
    protected function _company_url($row)
    {
        ...
        $link = K::M('helper/link')->mklink('gong/chang',array($row['company_id']), array(), 'city:'.$row['city_id'], $site['rewrite']);
        return $link;
    }    
