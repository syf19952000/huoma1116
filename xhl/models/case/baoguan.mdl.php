 ...
 * 人要活得优雅,代码更需要优雅
 * $Id: baoguan.mdl.php 2016-1-13 14:11:00  xinghuali
 */
...
class Mdl_Case_Baoguan extends Mdl_Table
{   
  
    protected $_table = 'case_baoguan';
    protected $_pk = 'baoguan_id';
    protected $_cols = 'baoguan_id,case_id,title,photo,size,orderby';
    protected $_orderby = array('orderby'=>'ASC', 'baoguan_id'=>'DESC');
    ...
    public function update($baoguan_id, $data, $checked=false)
    {
        if(!$checked && !$data = $this->_check($data,  $baoguan_id)){
            return false;
        }
        return $this->db->update($this->_table, $data, $this->field($this->_pk, $baoguan_id));
    }
    ...
    public function upload($case_id, $attach)
    {
        ...
        if($baoguan_id =$this->db->insert($this->_table, $data, true)){
            $data['baoguan_id'] = $baoguan_id;
            return $data;
        }
        return false; 
    }
    ...
    public function delete($pids, $force=false)
    {
        ...
        $albums = $shops = $baoguan_ids = array();
        foreach($items as $item){
  			@unlink('czfiles/'.$item['photo'].'_small.jpg');
  			@unlink('czfiles/'.$item['photo']);
            $baoguan_ids[] = $item['baoguan_id'];
        }
        if(!empty($baoguan_ids)){
                $sql = "DELETE FROM ".$this->table($this->_table)." WHERE " . self::field($this->_pk, $baoguan_ids);
    ...
    public function _check($data, $baoguan_id=null)
    {
        unset($data['baoguan_id'],  $data['clientip'], $data['dateline']);
