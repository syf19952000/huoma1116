...
class Mdl_Case_Moxing extends Mdl_Table
{   
  
    protected $_table = 'case_moxing';
    protected $_pk = 'moxing_id';
    protected $_cols = 'moxing_id,case_id,title,photo,size,orderby';
    protected $_orderby = array('orderby'=>'ASC', 'moxing_id'=>'DESC');
    ...
    public function update($moxing_id, $data, $checked=false)
    {
        if(!$checked && !$data = $this->_check($data,  $moxing_id)){
            return false;
        }
        return $this->db->update($this->_table, $data, $this->field($this->_pk, $moxing_id));
    }
    ...
    public function upload($case_id, $attach)
    {
        ...
        if($moxing_id =$this->db->insert($this->_table, $data, true)){
            $data['moxing_id'] = $moxing_id;
            return $data;
        }
        return false; 
    }
    ...
    public function delete($pids, $force=false)
    {
        ...
        $albums = $shops = $moxing_ids = array();
        foreach($items as $item){
  			@unlink('czfiles/'.$item['photo'].'_small.jpg');
 			@unlink('czfiles/'.$item['photo']);
            $moxing_ids[] = $item['moxing_id'];
        }
        if(!empty($moxing_ids)){
                $sql = "DELETE FROM ".$this->table($this->_table)." WHERE " . self::field($this->_pk, $moxing_ids);
    ...
    public function _check($data, $moxing_id=null)
    {
        unset($data['moxing_id'],  $data['clientip'], $data['dateline']);
